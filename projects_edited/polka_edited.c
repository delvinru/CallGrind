...
char pop_op(StackNodePtr* topPtr)
{
	add_sensor("pop_op()", callgrind_checker++);
...
void push_op(StackNodePtr* topPtr, char oper)
{
	add_sensor("push_op()", callgrind_checker++);
    StackNodePtr newPtr;
...
float pop_num(StackNumberPtr* topPtr)
{
	add_sensor("pop_num()", callgrind_checker++);
...
void push_num(StackNumberPtr* topPtr, float num)
{
	add_sensor("push_num()", callgrind_checker++);
...
{
	add_sensor("priority()", callgrind_checker++);
    switch (sym) {
        case '(':
	{callgrind_checker--;
            return 0;
...
void create_exp(char* exp, char* output, int n)
{
	add_sensor("create_exp()", callgrind_checker++);
...
void calc_exp(char* exp)
{
	add_sensor("calc_exp()", callgrind_checker++);
